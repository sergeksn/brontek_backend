...
// удаление из белых ботов
foreach ($ab_se as $k => $v) {
if (in_array('.',$v)) {
$k = $antibot_db->escapeString($k);
$del = $antibot_db->exec("DELETE FROM rules WHERE rule = 'white' AND comment LIKE '%".$k."%';");
}
}
...
<p><small>'.abTranslate('Rules by user-agent, country, language, referrer, ptr - you can add only to the black list.').'<br />
<span style="color:red;">'.abTranslate('Blocking by user-agent').'</span> - '.abTranslate('add a full string like:').' Mozilla/5.0 (Windows NT 10.0; WOW64; rv:42.0) Gecko/20100101 Firefox/42.0<br />
...
if (filter_var($echo['search'], FILTER_VALIDATE_IP)) {
$links = '/ <a href="?'.$abw.$abp.'=ip&ip='.$echo['search'].'" target="_blank" rel="noopener">whois</a>';
} else {
$links = '';
}
...
<td><small>'.$echo['comment'].' '.$links.'</small></td>
...
<p>
<form action="?'.$abw.$abp.'=clearrulesban" method="post" style="display: inline-block;">
<input name="'.$abp.'" type="hidden" value="rules">
<input style="cursor:pointer;" class="btn btn-sm btn-danger" type="submit" name="submit" value="'.abTranslate('Remove all many_buttons_ban rules').'">
</form>

<form action="?'.$abw.$abp.'=clearrules" method="post" style="display: inline-block;">
<input name="'.$abp.'" type="hidden" value="rules">
