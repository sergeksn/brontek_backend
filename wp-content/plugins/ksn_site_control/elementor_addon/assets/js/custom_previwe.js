//функции
(function($) {
    let text_img_error = ". Приносим извинения, но увы изображение не удалось загрузить. Попробуйте обновить страницу. Будьте на позитиве =)",
    ...
    data_site.top_menu = function() {
        ...
        open_close_menu_top = function() {
            ...
            function touch_menu_open_close(e) {
                ...
                menu_block.height($(window).height() - 70); //для того чтоб меню занимало всю высоту экрана
    ...
    //
    //
    data_site.o_c_modal_bot = function() {
        ...
        function scroll_positon_check() {
            if (proverka_scrollup == 1) { //проверяем не нужно ли менять поведение кнопки при скроле
                if ($(document).scrollTop() > 500) { //показываем кнопку scrollup только если документ прокручен более чем на 500px
                    ...
                    if (!scrollup.hasClass("dekoder_max") && scrollup.find("img").attr("data-type")) {
        ...
        function open_modal_window(e) {
            ...
            overlay.fadeIn(400, function() {
                modal_window.css({ "display": "block" }).animate({ opacity: 1, top: css_top }, 200, function() {
                    if (!modal_close_img.parent().hasClass("dekoder_max") && modal_close_img.attr("data-type")) {
    ...
    //
    //
    data_site.o_c_razvorot_block = function() {
    ...
    data_site.adaptiv_tabls = function(wrap) {
        wrap.find(".adaptiv_tabls").cardtable();
    };
    //адаптивные таблицы
    ...
    //
    //
    //ЗАПУСК ТРЕБУЕМЫХ ФУНКЦИЙ ДЛЯ КАЖДОГО ВИДЖЕТА С СООТВЕТСТВУЮЩИМИ ПАРАМЕТРАМИ
    //
    //
    data_site.media_load_for_elementor = function(wrap, media_type) {
        if (media_type == "images") {
            let images = wrap.find("img[data-type='img_content']"),
                img_maket = wrap.find("img[data-type='img_maket']");

            images.on("load", function() { $(this).parent().addClass("loaded"); });
            img_maket.on("load", function() { $(this).parent().addClass("loaded"); });

            images.on("error", data_site.img_erroe_load);

            data_site.add_to_load_media(img_maket, "images");
            data_site.add_to_load_media(images, "images");
        } else if (media_type == "video" || media_type == "maps") {
            let video = wrap.find(".block_video .video_wrapper .video_ytube"),
                maps = wrap.find(".block_map .map_wrapper .map_fpame");

            data_site.add_to_load_media(video, "video");
            data_site.add_to_load_media(maps, "maps");
        } else if (media_type == "insta_gallery") {
            let insta_gallerys = wrap.find(".instagram_img_sektion .instagram_img");
    ...
    $(window).on('elementor/frontend/init', () => {
        function header(scope) {
            data_site.top_menu();
            if (scope.hasClass("elementor-element-editable")) {
                return;
            }
            data_site.media_load_for_elementor(scope, "images");
        };

        function footer(scope) {
            data_site.o_c_modal_bot();
            if (scope.hasClass("elementor-element-editable")) {
                return;
            }
            data_site.media_load_for_elementor(scope, "images");
        ...
        function unifi_blok_1(scope) {
            //проверяем редактируется ли виджет, если редактируется то завершаем выполение функции, чтоб избежать долгих вычислений при каждом наборе новой буквы в текстовом поле, например =)
            data_site.media_load_for_elementor(scope, "video");
            data_site.media_load_for_elementor(scope, "maps");
            if (scope.hasClass("elementor-element-editable")) {
                return;
            }
            data_site.adaptiv_tabls(scope);
            data_site.media_load_for_elementor(scope, "images");
        };

        function instagram_galery(scope) {
            data_site.media_load_for_elementor(scope, "insta_gallery");
        };

        function razvorot_block(scope) {
            data_site.o_c_razvorot_block();
            if (scope.hasClass("elementor-element-editable")) {
                return;
            }
            data_site.media_load_for_elementor(scope, "images");
        ...
        function animate_diskont_img_text(scope) {
            if (scope.hasClass("elementor-element-editable")) {
                return;
            }
            data_site.media_load_for_elementor(scope, "images");
        ...
        function animate_img_text(scope) {
            if (scope.hasClass("elementor-element-editable")) {
                return;
            }
            data_site.media_load_for_elementor(scope, "images");
        ...
        function contact_map_img(scope) {
            data_site.media_load_for_elementor(scope, "maps");
            if (scope.hasClass("elementor-element-editable")) {
                return;
            }
        ...
        function parent_uslugi_icon(scope) {
            if (scope.hasClass("elementor-element-editable")) {
                return;
            }
            data_site.media_load_for_elementor(scope, "images");
        ...
        elementorFrontend.hooks.addAction('frontend/element_ready/header_widget.default', header);
        elementorFrontend.hooks.addAction('frontend/element_ready/footer_widget.default', footer);
        elementorFrontend.hooks.addAction('frontend/element_ready/unifi_blok_1.default', unifi_blok_1);
        elementorFrontend.hooks.addAction('frontend/element_ready/instagram_galery.default', instagram_galery);
        elementorFrontend.hooks.addAction('frontend/element_ready/razvorot_block.default', razvorot_block);
        elementorFrontend.hooks.addAction('frontend/element_ready/animate_diskont_img_text.default', animate_diskont_img_text);
        elementorFrontend.hooks.addAction('frontend/element_ready/animate_img_text.default', animate_img_text);
        elementorFrontend.hooks.addAction('frontend/element_ready/contact_map_img.default', contact_map_img);
        elementorFrontend.hooks.addAction('frontend/element_ready/parent_uslugi_icon.default', parent_uslugi_icon);
        elementorFrontend.hooks.addAction('frontend/element_ready/randomayzer.default', randomayzer);
        //elementorFrontend.hooks.addAction('frontend/element_ready/footer_widget.default', footer);//срабатывает при выводе определённого виджета
        //elementorFrontend.hooks.addAction( 'frontend/element_ready/widget', dobavit_nuznie_funkcii );//срабатывает при выводе каждого виджета
